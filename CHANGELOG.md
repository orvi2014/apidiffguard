...
- **The console navigation works on a phone.** The header gives its width to the navigation (settings moves into the account menu, and "add endpoint" stays in page headers), the row fades at whichever edge has more destinations, and the current page scrolls into view
...
### Fixed
- **The expanded JSON editor can be typed in again.** In the free tools' full-screen view, focus jumped to the Exit button after the first character, so the second character was lost and the next space closed the editor. The focus trap re-armed on every keystroke because each render handed it a new close callback. Expanding now also puts the cursor in the editor rather than on the Exit button, and Escape returns you to the button you opened it from
- **Copy in the JSON tools says when it fails.** A refused clipboard used to fail silently; the button now reads "Copy failed", and "Copied" is announced to screen readers
- **The "… checks today" placeholder in the console footer was invisible**, drawn in the panel's own grey. A test now fails the build if that token mistake comes back
- **Console header buttons are no longer nested inside links**, so each Overview action is one tab stop and screen readers announce it once
