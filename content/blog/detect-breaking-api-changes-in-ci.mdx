...
Under the hood that’s a JSON compare with severity rules — the same engine behind our free [JSON Diff tool](/tools/json-diff) and the open-source `@apidiffguard/diff` package.
