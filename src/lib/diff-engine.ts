...
/** Preferred identity keys when matching array items across reorders. */
...
export function compareJson(
  ...
  const walk = (a: unknown, b: unknown, current: string) => {
    ...
    // null ↔ non-null is a nullability / contract risk
    ...
    if (ta === "array" && Array.isArray(a) && Array.isArray(b)) {
      if (
        arrayIdentity &&
        canUseArrayIdentity(a) &&
        canUseArrayIdentity(b)
      ) {
  ...
  if (Array.isArray(value)) {
    return {
      ...
      children: value.map((v, i) => {
        const id = arrayItemIdentity(v);
        const childPath = id ? `${path}[${id}]` : `${path}[${i}]`;
        return buildJsonTree(v, childPath, id ?? String(i), changeMap);
      }),
    };
  }
